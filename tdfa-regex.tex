...
\item The available transition $q_7\rightarrow q_8$ is evaluated:\begin{enumerate}
	...
	\item 	$h\leftarrow\pos+1$. This is the position after the ``,'', because this is not part of the capture
			group.
	\item	$H_3$ is a copy of $H_2$ with $h$ on position 5. Since $h$ is pointing to the old history, it
...
\item $(q_5, (h_1, h_2, h_9, h_{10}, h_5, h_6))$ is popped from $high$.
\item $R=\{(q_4, (h_1, h_2, h_3, h_4, h_5, h_6)),\allowbreak (q_5, (h_1, h_2, h_9, h_{10}, h_5, h_6))\}$
\item No transitions are visited from $5$.
\item $(q_3, (h_1, h_2, h_3, h_4, h_5, h_6))$ is popped from $low$.
\item $q_3\rightarrow q_4$ is visited \begin{enumerate}
	\item $q_4$ is in $R$, so no further steps are taken.
\end{enumerate}
