...
many other tasks involving structured data in strings. Their efficiancy make 
them useful even for big data sets. Standard algorithms run in 
...
Regular expressions are typically described by a string of literal characters
to match and some special constructs specific to them. With this they can 
describe any regular language\cite{Sipser2005} if one only considers 
\emph{match} vs \emph{non-match} and can extract sub-patterns when capture 
groups are added.
...
negate their content, so \texttt{[\string^a-g]} would match anything except a thru g.
...
either zero or one repetitions of the preceding element, prefering to have 
...
repetition of the preceding element, including zero repetitions, prefering as 
...
\paragraph{Capture groups} Patterns enclosed by parens are treated as a 
single element, thus \texttt{(ab)*} captures $ab$, but not $aba$. More 
imporantly for us is that after the match, the capture groups can be extracted:
\texttt{a(b*)c} when matching $abbbc$ can extract $bbb$ and the empty string 
when matching $ac$. Note that the capture groups can be nested and the 
semantics differ for \textsc{Posix} and tree regex: In \textsc{Posix} the 
regular expression \texttt{a((bc+)+)} on the string $abcbccc$ will give $bcbccc$ 
for the outer capture group and $bc$ for the inner capture group -- the 
leftmost occurence of outer capture groups is kept and within that substring, 
the leftmost occurence of the inner group is kept. In tree regex, all 
occurences are kept and returned in a tree structure: The outer capture group 
contains $bcbccc$ and both inner matches $bc$ and $bccc$.

\section{(Non-)deterministic finite-state automata}
As they are heavily used throughout this paper, let us recall what
non-deterministic finite-state automata (NFAs) and deterministic finite-state
automata (DFAs) are. A DFA is a state machine that will walk over a finite
...
NFAs differ from DFAs in that for some input character and some state, there
...
will magically guess the correct one. \autoref{fig:example-automaton} shows an
example of an NFA's transition graph.
...
\caption{Thompson~\cite{Thom68a} construction of the
...
\subsection{Lazy DFA compilation}
...
regular expression matcher. This is unfortunately the best we can hope for, 
as can be seen in \Secref{lower-bound}.
...
Kearns~\cite{Kear91a} describes the first so-called linear-time algorithm for
RE parsing.  Dub\'e and Feeley~\cite{Dube00a} produce parse trees in so-called
linear time using lists as their data representation. Nielsen and
Henglein~\cite{Niel11a} improve on it by storing the trees in bit-coded form.
Neither produces a greedy or non-greedy parse.  Grathwohl~\cite{Grat13a}
produce a greedy parse in two passes over the input.  In comparison, our
approach allows full control over greediness, even for subexpressions, thus
generalizing over all previous approaches, while matching the running time 
guarantees of regular expression matching. It runs in only one pass and
supports character ranges, lazy compilation, and compactified DFA
representation for practical performance.

\chapter{Algorithm}
\seclabel{algo}

The algorithm we present is a extension of the non-deterministic finite state 
automaton (NFA) matching algorithm for regular expressions with added 
logging of the start and end of capture groups, as introduced by
...
complete algorithm, which is tight for NFA algorithms as seen in
\Secref{lower-bound}. Since this is not a purely theoretical paper, there are
also practical considerations such as caching current results, just-in-time
...
\While{high and low are not both empty}
	...
	\algstore{onestep}
\end{algorithmic}
\caption{\label{onestep}$\mbox{onestep}(\mbox{NFA}, a, \mbox{pos}, Q)$: Compute
...
work correctly even if both occur during the onestep execution.} will survive.
...
\section{Data structures}	\seclabel{treap}\seclabel{data-structures}
