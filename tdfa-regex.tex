		...
		\For{$i=0\dots length(H)-1$}{
			...
			\eIf{$H(i)$ is in $m$ as a key already and does not map to $H'(i)$}{
				Fail\;
				 ...
				 \tcp{Hypothesize that this is part of a valid map}
...
can reach their destinations first.
...
\item Initialize $R=[]$, the DFA state under construction. We will now proceed to add threads to this list in the order in which they need to be run in the next run of \emph{oneStep} in order to obey greediness.
