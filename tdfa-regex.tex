...
\usetikzlibrary{arrows,automata,shapes,positioning,snakes}
...
to optain a more manageable format. As an example, consider log files.  As
...
to match and some special constructs specific to the regular expression syntax.
With this they can describe any regular language\cite{Sipser2005} if one only
considers \emph{match} vs \emph{non-match} and can extract sub-patterns when
capture groups are added.
...
right path is an important efficiancy feature and for all capturing 
...
the group, while \texttt{a(.*?)c?} will capture only $b$. This is because the 
grammar is ambiguous without specifying the greedyness of the match -- both 
...
suspend itself and continue later -- it is similar to a thread, but it is 
not executed in parallel. In the context of NFA a co-routine contains the 
...
algorithm~\ref{alg:coroutine-tagged}. To see how this is a generalisation of 
...
versions, where each $set$ call forks off a new node, as seen in \Figref{version-tree}.
...
\subsection{Copy-on-write array}
...
\emph{overflow} in a subtree, which will trigger relabeling.
...
amortized insert. This can be resolved using a technique known as \emph{indirection}.

\subsubsection{Indirection}
Indirection is a method introduced by Willard\cite{Will82a} to eliminate 
annoying $\log$ factors under certain conditions such as the one just encountered.
