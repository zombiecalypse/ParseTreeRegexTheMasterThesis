...
\texttt{(ab)*} captures $ab$, but not $aba$. More imporantly for us is that
...
\textsc{Posix} and tree regex: In \textsc{Posix} the regular expression
\texttt{a((bc+)+)} on the string $abcbccc$ will give $bcbccc$ for the outer
capture group and $bc$ for the inner capture group -- the leftmost occurence of
outer capture groups is kept and within that substring, the leftmost occurence
of the inner group is kept. In tree regex, all occurences are kept and returned
...
the parsing is ambiguous without specifying the greedyness of the match -- both
...
worse than the state of the art, at $O(n^{2}m)$ \cite{Sucanlz12a}.
...
\nes{POSIX matching? same conference of Dube. Please check all. Quote more. 
Expand all. 
There's a paper on using parse trees to achieve POSIX. There's a rsc groups 
post on why posix sucks.}
