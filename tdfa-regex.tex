...
\item [$c\uparrow(h)$] $c\downarrow(h)$ Prepends linked list $h$ with \chg{an extra}{a new} node, which becomes the new head.
...
\ugh{It is easy to see that}, after matching, given the history of every tag, the entire match tree can be reconstructed.
...
The following examples will illustrate algorithm~\ref{onestep} and use the
following notation:
...
\begin{itemize}
	...
	\item 	A history is denoted $h_i$. Since a history is actually a
			...
			\on{Oh. Before $h$ was just the head of a history.}
	\item	Each NFA state has an array of $2n$ histories, where 
			...
			the capture group $i$, and at position $2i+i$ is the closing history.
				\on{Shouldn't it be $2i+1$?}
			An NFA state of the regular expression \texttt{(.+)} therefore
	...
	\item	\chg{The algorithm uses $high$ and $low$ as stacks and \emph{push} means
			to put an element at the topmost position. \emph{Popping} 
			means to remove and retrieve the topmost element of the stack.}
			{The algorithm uses two stacks, called \emph{high} and \emph{low}.}
