...
after no lazy compilation has occurred in a while. \chg{Worse,}{Furthermore,} compactification
can be undone if needed. We'll get back to these details in TODO
LATER. Let's discuss the stages in turn, starting with step 2, since
...
\on{What are the up and down arrows for? I assume "+" and "$-$" are high and low priority ...  What about the $\tau$s? Are they tags? Why is there a $\tau_3\downarrow$ but no $\tau_3\uparrow$?}
}
\end{figure}
...
\on{Somewhere here remind us that "T" stands for "tagged". What are the tags for?}

\global\long\def\naturals{\mathbb{N}}
\global\long\def\integers{\mathbb{Z}}
\global\long\def\pos{\mathbf{\mathbf{p}}}
Our algorithm is a modification of Laurikari's algorithm \cite{Lauri00a},
...
\end{comment}
Our description adapts the thread metaphor of \cite{Cox07a}, where
