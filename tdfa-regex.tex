...
\section{Benchmark}
\seclabel{benchmarks}
While measuring the performance of our tool is easy, it is not trivial to find a base-line to compare it against.
As we will see in \Secref{related}, there is a surprising dearth of non-backtracking regular expression engine that can extract nested capture groups -- nevermind extracting entire parse trees! 

If a Java programmer wants to extract the parse tree described by a regular expression without using our framework,
 the pragmatic choice is between some parser framework, perhaps a combinatorial parser such as JParsec, and writing the parser by hand.



Compared to a back-tracking implementation, 


  regular expression engines that allow extracting capture groups, nevermind the entire matching trees,
are back-tracking

\begin{figure}[h]
\includegraphics[width=0.75\linewidth]{graphs/pathological}
\caption{\figlabel{patho} Matching $\textit{a?}^n\textit{a}^n$ against input $\textit{a}^n$. Bottom line is our approach, top line is java.util.regex.Pattern.}
\end{figure}
\figlabel{patho}
\begin{table}
