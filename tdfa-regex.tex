...
Anything that will be pushed on the high stack \ugh{or the low stack} \on{huh?} will be dealt with
first.
...
\item $(q_7, H_2)$ is pushed to \emph{high} \ugh{as fed} \on{not clear how we see this -- where is this bit of information and what does it mean? Or do you mean that the high stack only cottons fed threads, and low contains hungry ones?}, and we continue the main loop.
...
\item The available transition $q_7\rightarrow q_8$ is evaluated:\begin{enumerate}
	...
	\item 	$h\leftarrow\pos+1$. This is the position after the ``,'', because the comma was eaten before the capture
			group starts.
	\item	Create $H_3$ as a copy of $H_2$ with $h$ on position 4 (indices are 0-based \on{actually you are inconsistent, since you start with $h_1$, not $h_0$}). \on{cost?}
...
\item $q_4\rightarrow q_3$ is visited. \ugh{Note that $q_3$ has not yet been seen.} \on{huh?}
\begin{enumerate}
...
\on{Not sure how to interpret the result. Do we really have all these threads now? Is this correct?}
