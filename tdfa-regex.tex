...
of the matcher. \del{It is well-defined at all times since all threads
are executed in lock-step, such that at any point in time, all
threads are at the same position.} \ml{by here i have no clue what a thread is. maybe just drop this sentence}
...
that the DFA is \emph{lazily compiled}. Note that the DFA transition
table is different from the NFA transition table, in that the NFA
transition table contains $\varepsilon$ edges and may have more
than one transition from one state to another, for the same input
range.
...
With these ingredients in place, the entire matching algorithm is straightforward. 
In a nutshell, we see if the current input appears in the transition table. Otherwise,
we run \texttt{oneStep}. If the resulting state is mappable, we map.  At any rate, we add an entry to the transition table. 
More formally, we can see this in algorithm \ref{interpret}.
...
The algorithm uses these two stacks to handle the scheduling of the
threads. The LIFO\footnote{last-in-first-out} property ensures that
threads preferably are run until they can't follow further
$\varepsilon$-edges. The \emph{high} stack takes strict priority.
...
parser by hand. As we will see in \Secref{benchmarks}, the performance of general-purpose 
...
JParsec, which, as also seen in \Figref{abc}, performs on par with
...
\section{Conclusion}
When a regular expression engine matches input, it can construct
the entire parse tree of this matching, at no or little additional
computational cost. All source code and all benchmarks are available
under a free license on github.
