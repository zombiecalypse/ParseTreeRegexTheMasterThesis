...
$O(n\, m)$, where $n$ is the length of the string to be matched
...
\paragraph{Literals} The simplest form of regular expressions are literal
characters like \regex{a}.
...
the string in order. For example \regex{a[bc]} matches \str{a} followed by either
\str{b} or \str{c}.
...
\paragraph{Alternation} Patterns separated by the pipe symbol \texttt{|} can
...
Patterns enclosed by parentheses are treated as a single element, thus
...
Because backtracking is so intuitive we will use it as the definition of a
correct behaviour throughout this thesis.
...
of \emph{co-routines}. Co-routines are procedures that has the ability to
suspend themselves and continue later -- they are similar to threads, but co-routines
don't need to be executed in parallel and are usually manually scheduled. In
order to resume their work later, they contain some kind of memory that is
specific to them.

In the context of NFAs, a co-routine contains the current state and has access
to the transition graph\footnote{Later more memory will be added to store the
sub-matches.}. When reading a string, the NFA maintains a set of co-routines
and run them in lock-step. This means that each co-routine reads the
current character and spawns new co-routines for all possible transitions. When
reading a character, a co-routine must consume it, but might follow
$\varepsilon$-transitions before. We model this by following
$\varepsilon$-transitions in order, and whenever we encounter a transition that
can consume the current character, the state following it is added to the next
scheduling cycle.
...
$\varepsilon$-transitions -- therefore a DFA state is a set of NFA states. The
transitions simulate a step in the original NFA, so they point to another set
of states.  After the compilation is done, matching the string is $O(n)$ time.
...
intertwine the compilation and the matching to only expand new DFA states that
...
running in lock-step with each step consuming exactly one character of the 
input string\footnote{The empty string can be modelled as containing only the 
`\textbackslash 0' character.}
This model already implies that some form of instructions are executed on a
...
Section~\ref{sec:tfsm} will explain how this can be simulated in TNFA interpretation,
...
interrupted by the need to consume another character. This prioritization is 
achieved by using a buffer, that reverses the order of high-priority runs.

\begin{algorithm}
  \begin{algorithmic}
  \LineComment{Returns a list of co-routines that consumed the character}
  \Function{$run_{tagged}$}{coroutines, char}
  \LineComment{$coroutines$ is a list of co-routines in order as returned here.}
  \LineComment{$char$ is a character}
    \State Put all coroutines on the $low$ stack
    \State Initialize empty $buffer$ stack
    \State Initialize empty list $R$ \Comment{the returned list of co-routines}
...
Conceptually, our approach consists of four stages:
\begin{enumerate}
...
compilation has occurred in a while. Also compactification can be
undone if needed. Since the essence of the algorithm is step 2 and 3,
we start with them and proceed to see steps 1 and 4 as part of the implementation.
...
An automaton for the expression \regex{.} is constructed. The expression
...
\item[DFA states] are denoted by a capital letter, e.g. $Q$, and
  ...
  \begin{equation*}\begin{split}Q=[&(q_1, (([0], [12]), ([9, 1], [10, 2]), ([], []))),\\
	...
	have the same matches. The order of the co-routines is relevant and a DFA 
  state is thus a list of NFA states.
\item[Histories] are linked lists, where each node stores a position in the
  input text.  The head is mutable, and the rest is immutable. Therefore, histories
  can share any node except their heads. We write $h=[x_1, \dots, x_m]$ to
  describe that matches have occurred at the positions $x_1, \dots, x_m$.
\item[Co-routines] are denoted as pairs $(q_i, h)$, where $q_i$ is some
  ...
  (h_{2n-1}, h_{2n}))$, history $h_1$ is the history of the openings
	of the first capture group, and $h_2$ is the history of the closings of
  the first capture group, and so on. The visual pairing of the histories is in
  order to visualize which histories denote start and end of which capture group.
\item[Transitions] are  understood to be between NFA states, so
	$q_1\rightarrow q_2$ means a transition from $q_1$ to $q_2$.
\end{description}

Take for example the regular expression \regex{(..)+} matching
pairs of characters, on the input string \str{\overset{0123}{abcd}}. The history array of the
...
\begin{algorithmic}[1]
  \LineComment{Update the co-routine, interpreting the tag}
  ...
  \EndFunction
\end{algorithmic}
\caption{ \label{alg:interpret} Interpretation of the tags.}
\end{algorithm}
...
read. The \emph{buffer} variable is a detail that ensures that the correct 
order of co-routines is produced. If our procedure is consistently used, the
prioritization will lead to a correct match.
...
\begin{algorithm*}
  \begin{algorithmic}
  \Function{$run_{tnfa}$}{coroutines, char}
    \State Put all coroutines on the $low$ stack
    \State Initialize empty $buffer$ stack
    ...
    \While{the stacks aren't both empty}
      ...
      \For{all $\varepsilon$-transitions from $c$ to state $s$ with tag $t$}
        \If{no co-routine in state $s$ exists in coroutines}
          \State copy the co-routine $c$ to $c'$
          \If{$t$ is open tag of group $i$}
            \State $set(index+1, c'.histories[i].left)$
          \EndIf
          \If{$t$ is close tag of group $i$}
            \State $set(index, c'.histories[i].right)$
            \State commit $c'.histories[i].left$ and $c'.histories[i+1].right$
  ...
  \EndFunction
  \end{algorithmic}
  \caption{\label{alg:coroutine-tagged-complete}Complete transition execution 
    as implemented by us. This is algorithm~\ref{alg:coroutine-tagged} with 
    algorithm~\ref{alg:interpret} inlined. See appendix~\ref{python-match} for an actual
...
DFA states -- the equality of the contained NFA states doesn't care about the 
order in which they are visited and furthermore it doesn't respect that two 
...
implementation using this to the author's knowledge is Pike~\cite{Pike87a} in
...
The correct handling of greediness (not of non-greediness) was implemented by
Kuklewicz~\cite{Kukl07a} for the Haskell implementation\footnote{Or free
interpretation, since Laurikari leaves the matching strategy open.} of
Laurikari's algorithm. This too was only published in source code, to a great
loss for the academic community.

Cox calls Laurikari's TDFA a reinvention of Pike's algorithm, but while that 
is in part true, Laurikari introduces the mapping step described in 
algorithm~\ref{findmapping}. This leads Laurikari's algorithm to contain 
fewer states and one would hope that this would lead to a better run-time than 
...
necessary to build a data structure that has the same interface as a
copy-on-write array:
...
Every version that is in a living co-routine can change and so a single 
version can fork into different directions. A way to visualize
the history is as a tree of versions, where each $set$ call forks off a new node, as
seen in \Figref{version-tree}.
...
child {
  ...
  were forked from the same state in the TNFA. The labels describe the relative
order of creation.}
...
itself before modifications are made:
...
size, thus lazily applying the modifications. For the purposes of this 
chapter, we will call this kind of data structure $LazyApply$.

Let's assume for the moment, that modifications only ever happen in the most
recent version of our array.
...
The amortized constant time for $get$ and $set$ arise from the fact that while
...
The problem of generalizing this approach to version trees is twofold: The
...
can split the modification buffer into two roughly equal parts. In order to do
...
find the relation between the version that is queried and the one that is
stored. If $t_{stored} < t_{queried}$, we apply the modification. This
requires us however to determine the relative positions of $t_{stored}$ and
$t_{queried}$ in the version tree, where the naive implementation would take
...
\begin{enumerate}
  \item allows an element to be inserted next to a known element in $O(1)$ time.
...
\begin{defn}
  ...
  starting counting from the leaves, which are level 0.
...
The correct parse is found if and only if after reading the whole string,
\begin{enumerate}
  \item the co-routine in the end state consumed all characters of the string
    (and only those) in order and \label{correct-consume}
  \item there is no co-routine that fulfills \ref{correct-consume} that took 
    ``later'' low-priority edges. This corresponds to the depth-first search of
    backtracking.
\end{enumerate} 
...
There is the possibilities of circles, so that the depth-first solution would 
loop. This can be seen for example in the regular expression \regex{(a*?)*}, 
where the preferred route in the graph is actually to capture an empty 
repetition of \regex{a}. We tweak the Thompson construction for this 
scenario, by giving a path to the logically next state after the automaton 
with the same priority as from the start node for the star-operator, because 
it is the only automaton where the start state competes with a complete run 
through the pattern. 
...
list of instructions. This structure allows for quick insertion of
...
compactification can be done early, and only seldom need to be undone.

\section{Intertwining of the pipeline stages}
The lazy compilation of the DFA when matching a string enables us
to avoid compiling states of it that might never be needed. This
...
in simple regular expressions, can take up a major part (25\% in our
...
1950~\cite{Sips05a}. This chapter first introduces some standard
...
expression parsing to be a linear problem -- linear in the length of the string 
...
competitive for a small and fixed $m$, but is prohibitively expensive for large $m$. 
The argument that the pattern is typically small seems circular to the 
author, because would the implementation focus on allowing large patterns, 
...
In this paper, we will consider the best known algorithms to be quadratic. It is 
...
for regular backtracking: \regex{(x*)*y} on the string $x^n$. Now consider

\newcommand{\opor}{\quad || \quad}
\newcommand{\bt}{\widehat{bt}}

\begin{align*}
  \widehat{bt}(x^n, \regex{(x*)*y}) &= \underbrace{\bt(x^{n}, 
  \regex{(x*)(x*)*y})}_{\text{1 repetition}} \opor \bt(x^{n}, \regex{y}) \\
  &= \bt(x^{n-1}, \regex{(x*)(x*)*y}) \opor \bt(x^{n}, \regex{(x*)*y}) \opor \bt(x^{n}, \regex{y}) \\
  &=\dots \\
  &= \bt(x^{0}, \regex{(x*)(x*)*y}) \opor \bt(x^{1}, \regex{(x*)*y}) \opor \dots \\
  &\qquad\opor  \bt(x^{n}, \regex{(x*)*y}) \opor \bt(x^{n}, \regex{y}) \\
  &= \bt(x^{1}, \regex{(x*)*y}) \opor \dots \opor  \bt(x^{n}, \regex{(x*)*y}) \opor \bt(x^{n}, \regex{y}) \\
  &= \bt(x^{1}, \regex{y}) \opor \underbrace{\bt(x^{1}, \regex{(x*)(x*)*y})}_{\text{memoized}} \opor \dots \\
  &\qquad\opor  \bt(x^{n}, \regex{(x*)*y}) \opor \bt(x^{n}, \regex{y}) \\
  &=\dots\\
  &= \bt(x^{n}, \regex{y}) \\
  &= \text{no match}
...
Nearly all backtracking can be avoided, because the branch has been evaluated 
before.
...
parsers for small input, but they cannot be used for big data sets. To
understand how they work, let's first look at PEG:
...
restriction to PEG simplify the problem: without ambiguity fewer
...
scenario where the input string is of unknown size or of size too big to fit
on a single machine.
...
from $M_1$ to $M_n$ in turn and discard them afterwards. Even if the parse tree
...
Our implementation & 225 & 252 & 273 & 32 & 327 & 352 & 400 & 421\tabularnewline
\bottomrule
\end{tabular}
...
Finally, a more realistic example, neither chosen to favor
...
\chapter{Conclusion}
Regular expressions make for lightweight parsers and there are many cases
