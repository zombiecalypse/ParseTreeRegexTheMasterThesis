...
one repetition. This means \regex{a?} will match the empty string or $a$, but nothing else.
...
concatenation. Therefore \regex{abc|xyz} would match $abc$ or $xyz$, but not
$abxyz$.

\subsection{Capture groups}
Patterns enclosed by parens are treated as a single element, thus
\regex{(ab)*} captures $ab$, but not $aba$. More importantly for us is that
...
\regex{a((bc+)+)} on the string $abcbccc$ gives $bcbccc$ for the outer
...
regular expression \regex{a(.*)c?} on the string $abc$ captures $bc$ in
the group, while \regex{a(.*?)c?} captures only $b$. This is because
...
expression \regex{(a)|(.)} reading the string $a$: depending on the path
...
automaton: Descend into the abstract syntax tree of the regular
expression and expand the constructs recursively. In comparison to the simple 
construction in figure~\ref{thompson-simple}, the forward transitions from 
the top state in the star operators should be surprising, but they are
necessary if $S$ has a prioritized path that captures the empty string: We
cannot return to the start state, because we expanded it already, but we can proceed
anyway.}
...
\begin{algorithmic}[1]
  \Function{$interpret$}{$t$, $c$}
    ...
    \If{$t$ is the character $a$}
      \LineComment{We can only pass the edge if we can consume the character}
      \If{$c$ is not fed and can current character is consumable by $a$}
        \State become fed
      \Else
        \State die
      \EndIf
    \EndIf
    \If{$t$ is $\varepsilon$}
      \LineComment{If we're hungry, we can't cross $\varepsilon$-transitions}
      \If{$c$ is not fed}
        \State die
      \EndIf
    \EndIf
    \If{$t$ is open tag of group $i$}
      ...
      \State $set(index+1, c.histories[i])$ \Comment{See \Secref{data-structures}}
    \EndIf
    \If{$t$ is close tag of group $i$}
      \State $set(index, c.histories[i+1])$
      \State commit $c.histories[i]$ and $c.histories[i+1]$
    \EndIf
...
The algorithm uses a \emph{stack} with the possibility to push a low
...
\item Fill the stack with hungry co-routines of all states of $Q$.
  Now, $\mathit{stack}=[(q_6, H_2), (q_3, H_1))]$, where the first element
...
\item We now flush \emph{buffer}:
  The DFA state to be constructed is $R=[(q_8, H_3), (q_7, H_2)]$,
  the \emph{buffer}$=[\ ]$.  Note that now, $R$ contains two co-routines in the
  reverse order in which they were discovered.
...
\end{enumerate}
\item We flush the \emph{buffer} again: The DFA state to be constructed 
  becomes $R=[(q_8, H_3), (q_7, H_2), (q_4, H_1)]$
