...
\begin{verbatim}
bt : Pattern -> String -> (Bool, [Modification], String)
bt(r*, s) = bt(rr*, s) | bt(\varepsilon, s)
bt(r*?, s) = bt(\varepsilon, s) | bt(rr*, s)
bt(r+, s) = bt(rr*, s)
bt(r+?, s) = bt(rr*?, s)
bt(r?, s) = bt(r, s) | bt(\varepsilon, s)
bt(r??, s) =  bt(\varepsilon, s) | bt(r, s)
bt(a|b, s) = bt(a, s) | bt(b, s)
bt(ab, s) = bt(a, s) + bt(b, s)
bt(Group i r, s) = (True, [WriteOpen i current], s) + bt(r, s) + (True, 
[WriteClose i current], s) 
\end{verbatim}
...
\begin{lemma}\label{foundat}
  The algorithm \emph{read} on page~\ref{alg:coroutine-tagged} matches the empty string
  correctly.
\end{lemma}
\begin{proof}
