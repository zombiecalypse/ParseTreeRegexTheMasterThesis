...
undone if needed. We'll get back to these details in \secref{compact}.
...
After parsing succeds, the list of all histories can then be used
...
$(h_1, h_2, \dots h_{2n-1}, h_{2n})$. Here, $h_1$ is the history
of the openings of the first capture group, $h2$ is the history of
the closings of the first capture group, and so on.
\end{description}
...
\caption{\label{onestep}$\mbox{onestep}(\mbox{NFA}, a, \mbox{pos}, Q)$: Compute the follow-up state for DFA state $Q$.}
\end{algorithm*}
...
follows: The NFA states are taken, in order, they eat one input
...
the DFA state \footnote{NFA states that do not have any outgoing
transitions except for $\varepsilon$ transitions can be omitted,
since the first step will only consider edges which consume the
next character of the input.}
\begin{align*}
Q=[
...
Execute instructions described in algorithm oneStep, when walking $\varepsilon$ transitions.\;
...
\ForEach{position $pos$ in $input$}{
	...
	\eIf{$T$ has an entry for $Q$ and $a$}{
		...
		Run $\mbox{oneStep}(Q,a)$ to find new state $Q'$ and instructions \;
		Run $\mbox{findMapping}(Q', T)$ to see if Q' can be mapped to an existing state $Q''$\;
		\eIf{$Q''$ was found}{
...
Note because java.util.regex achieves its back-tracking through recursion,
we had to set the stack size to one Gigabyte for it to parse the input.
...
that cannot backtrack, since an attacker can potentially force the VM to run 
out of stack space.
...
as free software, the algorithm wasn't published anywhere.  Sam
uses Thompson's NFA, but adds submatch tracking. This seems unfair
in that Laurikari's allows for far more aggressive reuse of old
states than Thompson allows. This should lead to Laurikari's TDFA
having fewer states, and therefore better performance, than even
Google's RE2\footnote{\url{https://code.google.com/p/re2/}}, which
uses Pike's algorithm.  This is not confirmed by the benchmarks by
