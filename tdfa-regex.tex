...
\end{figure*}
A complete list of constructions can be seen in figure \ref{fig:thompson-construction}.

\subsection{Capture groups}

To model capture groups in the NFA, we add \emph{commit tags} to
the transition graph. First, we construct an automaton for the
expression inside of the capture group.  For example, to produce
capture group \texttt{(.*?)}, we first produce a transition graph
for \texttt{.*?}. In \figref{example-automaton}, states 3, 4 and
all transitions between them are exactly the transition graph for
expression ``\texttt{.*?}''.  This transition graph has a well-known
entrance, state 3, and a well-known exit, state 4.  To model a
capture group, we add two more states, 2 and 5,  that will serve
as the new entrance and exit, respectively.  We then connect the
new entrance to the old entrance, 2 to 3, via an $\varepsilon$
transition that is tagged with an open commit, and similarly, we
connect the old exit to the new exit, 4 to 5, via an $\varepsilon$
transition that is tagged witha  close commit.  The commit tags are
labeled with the number of their capture group.

Now, whenever the NFA moves from one state to another via a transition
that is tagged with a commit, it keeps, for this commit, a history
of all positions in which it was triggered.  It is easy to see that
the entire match tree can be reconstructed from the list of histories.


\begin{figure}
\includegraphics[width=\linewidth]{graphs/lehrer_automaton}

\caption{\figlabel{example-automaton}
