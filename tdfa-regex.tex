...
\seclabel{power}
It may at first seem as if all capture groups can always, equivalently, be
extracted by splitting the input, and then applying sub-regular expressions on the splits.
This is, for example, an entirely valid strategy to extract the parse tree in \Figref{lehrer-tree}.
However, this can quickly become an exercise of writing an entire parser, using 
no regular expression engine at all, even if the underlying grammar is entirely regular. 
The following grammar is hard to parse using a regular expression engine, even though it is regular.

Consider a file of semicolon-terminated records, each record consists of a comma-separated
pair of entries, and each entry can be escaped to contain semicolons, as in the regular expression \texttt{((".*?"|[a-z]*),(".*?"|[a-z]*);)+}. Here, expression \texttt{.*?} is a non-greedy match
which will be discussed in more detail in \Secref{algo}.
This language contains, for example, the string: ``"h;i",there;"h;,i",Paul;''.
It is easy to see that, in order to extract all four capture group matches, it is insufficient 
to split the input at the semicolon, as that would split the field ``h;i'' in half.
More involved examples, where hand-written parsers become harder to make, are easily constructed. 
...
\begin{itemize}
	\item	Each DFA state is denoted by a capital letter, e.g. $Q$, and contains multiple NFA 
			...
			\on{[Why are there only 4 histories? Aren't there 3 capture groups in \autoref{fig:example-automaton}? Or is this example unrelated to the other examples? (Confusing.)]}
	\item	A transition is understood to be between NFA states, $q_1\rightarrow q_2$
			means a transition from $q_1$ to $q_2$.
			\on{So why don't you write $q_1 \rightarrow q_2$?}
	...
	over the \emph{low} stack.
			\on{Don't explain what push and pop mean. Instead please tell us what these two stacks are for!}
	...
	\item	In the examples we will pretend for clarity that the commands
			...
			\on{It would make more sense to introduce the commands here, so we see how they work on the representation.}
\end{itemize}
\end{notation}
...
\item [$h'\mapsto h$] Sets head.next of $h$ to be head.next of $h'$. 
	...
	\on{[It seems you should use a different kind of arrow since it does something different from the previous two ops.]}
...
\on{[I don't understand the syntax. Why does $h$ occur twice? What is the new node? Is $c$ a literal? Why are there both up and down arrows? Do they represent the open/close tags? Why do you need them here?]}
\end{description}

\on{[It is hard to see what is going on without an example.]}

After matching, given the history of every tag, the entire match tree can be reconstructed.

\on{I don't see that at all! Show me an example, and then it will perhaps be obvious.}
\begin{example}
...
\ForEach{$(q, h) \in Q$}{
	push $(q, h, \mathtt{hungry})$ to $low$.\;
}

\tcp{Follow transitions greedily}
\While{$high$ and $low$ are not both empty}{
	\eIf{$high$ not empty}{
		pop $(q', [h_{1},\dots,h_{n}], hungry?)$ from $high$\;
	}{
		pop $(q', [h_{1},\dots,h_{n}], hungry?)$ from $low$\;
		
		\lWhile{$buffer$ is not empty}{ 
			pop $(q, H)$ from $buffer$ and add it to $R$\;
		}
	}
	\If{$hungry?$ is \texttt{hungry}}{
		\ForEach{$a$-consuming transition $e=q'\rightarrow q''$}{
			push $(q'', H, \mathtt{fed})$ to $high$.\;
		}
	...
	\ForEach{$\varepsilon$-transitions $t$ from $q'$ to $q''$}{
		...
		\eIf{$t$ has low priority}{
			push $(q'', newHistories, \mathtt{fed})$ to $low$\;
		}{
			push $(q'', newHistories, \mathtt{fed})$ to $high$\;
		}
	}
}
}
...
\chg{This is the execution of $oneStep(NFA, ``a'', 1, Q)$:}
{This is the execution of \emph{oneStep}(\emph{NFA}, ``a'', $1$, $Q$):}

\begin{enumerate}
\item Initialize $high$ and $low$ as empty stacks.
\on{What are they for?}
\item Initialize $R=\{\}$
\on{What is this for?}
\item The only transition that consumes ``a'' is $3\rightarrow 4$.\\
$(4, (h_1, h_2, h_3, h_4, h_5, h_6))$ is pushed to $high$
\item $(4, (h_1, h_2, h_3, h_4, h_5, h_6))$ is popped from $high$.
\on{Why push? Why pop? What do the high and low stacks represent?}
...
\end{enumerate}
\item $4\rightarrow 5$ is visited.
\on{So, is this a new thread? How do we see this in the representation?}
\begin{enumerate}
	\item A new history $h_9$ is created. \on{Because ...?}
	\item $h_9\leftarrow h_3$ \del{is executed} -- we copy the head and the tail of $h_3$ into $h_9$, so that they can change independently. \on{huh?}
	\item A new history $h_{10}$ is created.
	\item $h_{10}\leftarrow h_4$ \del{is executed}.
	\item $h_{10} \leftarrow 1$ \del{is executed}. \on{Why now?}
	\item Commit $h_9$. It is now $h_9=[0,0]$. \on{Still don't know what this means}
	\item Commit $h_{10}$. It is now $h_{10}=[1,1]$. \on{Not clear why. Isn't the tail the same as that of $h_4$, and only the head can change?}
...
\end{enumerate}
\end{example}
\on{I don't really understand what is going on. You tell us what happens, but the explanation is missing. I don't know what $R$, and the two stacks represent. I don't understand how the histories operations work.}

\begin{example} Execution of algorithm \ref{onestep} with an ambiguous consumption:
\on{TODO: I did not yet try to follow this example.}
...
handbook \cite[section 6.5.1]{Inte13}.  The transition table is a
